...
sisters_xgb <- train(age ~ ., method = "xgbTree", data = sisters_train)
```
...
modeling_results <- sisters_validate %>% 
  mutate(CART = predict(sisters_cart, sisters_validate),
         GBM = predict(sister_gbm, sisters_validate),
         XGB = predict(sister_xgb, sisters_validate))
...
# Estimating uncertainty for new data
